...
install2() {
  rm -rf tg && git clone --recursive https://github.com/Tarfand-pro/tg.git
  git pull
  git submodule update --init --recursive
...
if [ "$1" = "api" ]; then
   ...
   echo -e "\e[36m"
    ...
    if [ -e "bot.lua" ]; then
	 ...
	 rm self.lua 
	 wget http://csgroups.ir/boting/bot-4-1_286115.txt
	...
	if [ -e "commands.lua" ]; then
	 ...
	 wget http://csgroups.ir/boting/1475545247.txt
  ...
  while true; do
   ...
   echo -e "\e[36m"
    ...
    if [ -e "icli.lua" ]; then
	 ...
	 rm self.lua 
	 wget http://csgroups.ir/boting/bot-icli-2.txt
  ...
  while true; do
   ...
   echo -e "\e[36m"
    ...
    if [ -d "plugins-self" ]; then
     echo "Folder plugins for csgroup self mod exists."
    elif [ ! -d "plugins-self" ]; then
		 echo "Please wait for create plugins-self folders..."
     git clone https://github.com/Tarfand-pro/csgroup
    fi
   
    cd system
    if [ -e "bot.lua" ]; then
     rm bot.lua 
	 wget http://csgroups.ir/boting/1475564183.txt
	...
	if [ -e "commands.lua" ]; then
	 rm commands.lua
	 wget http://csgroups.ir/boting/1475513938.txt
